...
def test_list_stress_scenarios(db_session):
...
def test_create_custom_scenario(db_session):
...
def test_run_stress_test_full_reval(db_session):
    # GET scenario
...
def test_run_stress_test_approximation(db_session):
    # GET scenario
    scenarios = client.get("/api/v1/stress-scenarios").json()
    scenario_id = next(s["id"] for s in scenarios if s["name"] == "RATE_UP_100BP")
...
def test_run_stress_test_spread_widening(db_session):
    scenarios = client.get("/api/v1/stress-scenarios").json()
...
def test_compare_stress_tests(db_session):
    scenarios = client.get("/api/v1/stress-scenarios").json()
...
def test_get_stress_history(db_session):
    response = client.get("/api/v1/stress-tests/portfolios/1/history")
    assert response.status_code == 200
    data = response.json()
    assert type(data) is list

def test_invalid_scenario_creation(db_session):
    payload = {
        "name": "INVALID",
        "scenario_type": "CUSTOM",
        "rate_2y_shock_bps": 0,
...
def test_delete_predefined_scenario(db_session):
    scenarios = client.get("/api/v1/stress-scenarios").json()
